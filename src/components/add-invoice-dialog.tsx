...
export function AddInvoiceDialog({
  ...
  const [numberPrefix, setNumberPrefix] = useState<string>(
    INVOICE_CONFIG.DEFAULT_NUMBER_PREFIX
  );
  ...
  const currentFullDate = getFullDate(
    selectedYear,
    selectedMonth,
    formData.day
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn("sm:max-w-md text-foreground", theme)}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {strings["current_prefix"] || "Current prefix:"} {numberPrefix}
              </p>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="day">{strings["day"] || "Day"}</Label>
